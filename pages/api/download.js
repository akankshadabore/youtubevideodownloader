...
const execPromise = promisify(exec);
 ...
 async function handler(req, res) {
  ...
  try {
    ...
    const title = titleRaw.trim().replace(/[<>:"/\\|?*]+/g, ''); // clean file name
    ...
    stream.pipe(res);
    stream.on('close', () => fs.unlinkSync(finalPath)); // remove after download
  } catch (error) {
